...
func (t *tcpTransport) Listen() (interface{}, error) {
	t.lock.Lock()
	c := t.conn
	t.lock.Unlock()
	if c != nil {
	...
	tcpAddr, err1 := net.ResolveTCPAddr("tcp", t.localAddr)
	if err1 != nil {
		return nil, err1
	}
	c, err := net.ListenTCP(t.network, tcpAddr)
	t.lock.Lock()
	t.conn = c
	t.lock.Unlock()
	return c, err
...
func (t *tcpTransport) Read(conn interface{}, buf []byte) (num int, src net.Addr, msg message, err error) {
	c := conn.(net.Conn)
	for {
		num, err = c.Read(buf)
		if err != nil {
...
func (t *tcpTransport) Write(conn interface{}, pkt []byte, dst net.Addr) error {
	c := conn.(net.Conn)
	if err := c.SetWriteDeadline(time.Now().Add(t.writeTimeout)); err != nil {
		return err
	}
...
func (t *tcpTransport) Close(_ interface{}) error {
	t.lock.Lock()
	defer t.lock.Unlock()
